...
<div class="top-bar">
    ...
    <div class="-intro-x breadcrumb mr-auto hidden sm:flex">
        ...
        <a href="/manager/{{ $page_name }}" class="breadcrumb--active">{{ $page_title }}</a>
    </div>
    <!-- END: Breadcrumb -->
    <!-- BEGIN: Мэдэгдэл -->
    ...
    <div class="intro-x dropdown w-8 h-8">
        <div class="dropdown-toggle w-8 h-8 rounded-full overflow-hidden shadow-lg image-fit zoom-in" role="button" aria-expanded="false">
            <img alt="{{ config('settings.site_name') }}" src="{{ ($manager->image == null) ? asset('dist/images/Blank-avatar.png') : asset('uploads/users/thumbs/'.$manager->image)}}">
        </div>
        <div class="dropdown-menu w-56">
            <div class="dropdown-menu__content box bg-theme-26 dark:bg-dark-6 text-white">
                <div class="p-4 border-b border-theme-27 dark:border-dark-3">
                    <div class="font-medium">{{ $manager->name }}</div>
                    <div class="text-xs text-theme-41 dark:text-gray-600">Код: {{ $manager->email }}</div>
                </div>
                <div class="p-2">
                ...
                <div class="p-2 border-t border-theme-27 dark:border-dark-3">
                    <a href="{{ url('manager/logout') }}" class="flex items-center block p-2 transition duration-300 ease-in-out hover:bg-theme-1 dark:hover:bg-dark-3 rounded-md">
